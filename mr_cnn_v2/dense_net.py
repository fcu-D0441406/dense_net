...
class Dense_net:
    ...
    def __init__(self,img_size,channel,class_num,k,grow_rate,trainable=True):
        ...
        self.drop_rate = 0.25
        self.weight_init1 = tf.initializers.truncated_normal(0.0,0.001)
    ...
    def roi_to_softmax_and_dx(self):
        ...
        self.final_box = tf.layers.dense(f2,4,kernel_initializer=tf.random_uniform_initializer(minval=-1e-2, maxval=1e-2))
        #print(self.final_box,self.cls_score_p)
    ...
    def roi_pooling_1(self,rpn_box):
        roi = list()
        cls = np.zeros([rpn_box.shape[0],2])
        fpn_size={'0':[self.pre3,56],'1':[self.pre2,28],'2':[self.pre1,14],'3':[self.pre0,7]}
        for i in range(rpn_box.shape[0]):
    ...
    def roi_pooling_2(self,rpn_box,rpn_num):
        ...
        def fn(xi):
            w = xi[2]-xi[0]
            h = xi[3]-xi[1]
            feature_num = 3+tf.cast(log2((tf.sqrt(w*h)/224.0)),dtype=tf.int32)
            #print(feature_num)
            feature_num = tf.clip_by_value(feature_num,0,3)
            def f1():
                return self.pre0
            def f2():
                return tf.cond(tf.equal(feature_num,2),f3,f4)
            def f3():
                return self.pre1
            def f4():
                return tf.cond(tf.equal(feature_num,1),f5,f6)
            def f5():
                return self.pre2
            def f6():
                return self.pre3
            fpn_net = tf.cond(tf.equal(feature_num,3),f1,f2)
            xi = xi/224.0
            r = tf.image.crop_and_resize(fpn_net,[[xi[0],xi[1],xi[2],xi[3]]],[0],crop_size=[14,14],method="bilinear")
            r = tf.layers.average_pooling2d(r,2,2,padding='SAME')
            return r
        ...
        roi.append(tf.map_fn(fn, rpn_box))
        print(roi)
        roi = self.Concatenation(roi,0)
        roi = tf.squeeze(roi)
