...
class Resnext:
    ...
    def normal_act(self,x,trainable):
        x = tf.layers.conv2d(x,x.shape[3],3,1,padding='SAME',kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),
                                kernel_initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    def scale_block(self,x,s,trainable): 
        block = list()
        split_dim = x.shape[3]//s
        pre_block = None
        for i in range(s):
            now_block = x[:,:,:,:split_dim]
            if(pre_block==None):
                block.append(now_block)
            else:
                pre_block = self.normal_act(pre_block,trainable)
