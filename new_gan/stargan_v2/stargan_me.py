...
def generator(input_img,style_code,scope='generator'):
    ...
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        for i in range(2):
            ...
            '''
            x = resblock(x,ch,3,1,scope_name='resblock_up'+str(i))
            ch = ch//2
...
def train():
    ...
    fake_img = generator(original_img,target_style_encoder)
    ...
    cycle_original_img_style_encoder = style_encoder(cycle_original_img)
    ...
    style_ls = style_loss(fake_style_encoder,target_style_encoder)+style_loss(cycle_original_img_style_encoder,original_style_encoder)
    
    #diverse_ls = diverse_loss(fake_img2,fake_img3)
    
    g_loss = generator_ls+cycle_ls+style_ls
    ...
    with tf.name_scope('opt'):
        ...
        e_opt = tf.train.AdamOptimizer(5e-4,name='e_optimizer',beta1=0.0,beta2=0.99).minimize(g_loss,var_list=e_vars)
        ...
        with tf.control_dependencies([g_opt, e_opt]):
        ...
        for i in range(1000):
            ...
            for j in range(dog_image_list.shape[0]):
                ...
                if(j%2==0):
                    ...
                    sess.run([g_opt,e_opt],feed_dict={original_img:dog_img,original_label:dog_label,
                                                                 target_img:cat_img,target_label:cat_label})
                else:
                    sess.run(d_opt,feed_dict={original_img:cat_img,original_label:cat_label,
                                                                 target_img:dog_img,target_label:dog_label})
                    sess.run([g_opt,e_opt],feed_dict={original_img:cat_img,original_label:cat_label,
                                                                 target_img:dog_img,target_label:dog_label})
            ...
            if(i%2==0):
                g_ls1,g_ls2,g_ls3,d_loss_,fake_img_,cy_img = sess.run([generator_ls,cycle_ls,style_ls,d_loss,fake_img,cycle_original_img],
                ...
                plt.imshow(cy_img[0])
                plt.show()
            print(i,'generator_loss,cycle_loss,style_loss',g_ls1,g_ls2,g_ls3,'d_loss',d_loss_)
