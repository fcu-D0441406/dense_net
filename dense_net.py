...
class Dense_net:
    ...
    def build_net(self,trainable):
        with tf.variable_scope('dense_net'):
            dense1 = tf.layers.conv2d(self.x,2*self.k,5,2,padding='SAME',kernel_initializer=self.mrsa_init)
    ...
    def dense_block(self,x,block_num,trainable):
        ...
        x = tf.layers.conv2d(x,ch,1,1,padding='SAME',kernel_initializer=self.mrsa_init)
    ...
    def bottleneck(self,x,trainable):
        x = tf.layers.batch_normalization(x,training=trainable)
        x = tf.nn.relu(x)
        x = tf.layers.conv2d(x,self.k*4,1,1,padding='SAME',kernel_initializer=self.mrsa_init)
        x = tf.layers.dropout(x,self.drop_rate)
        x = tf.layers.batch_normalization(x,training=trainable)
        x = tf.nn.relu(x)
        x = tf.layers.conv2d(x,self.k,3,1,padding='SAME',kernel_initializer=self.mrsa_init)
        x = tf.layers.dropout(x,self.drop_rate)
        return x
